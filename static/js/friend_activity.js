    ...
    function renderFriends(friendsList, isSearch = false) {
        ...
        if (friendsList.length === 0) {
            ...
            friendsList.forEach(friend => {
                ...
                html += `
                <div class="friend-item ${opacityClass}" style="display:flex;align-items:center;padding:10px 0;cursor:pointer;" data-user-id="${friend.id}" onclick="if(window.openChat){ window.openChat('${friend.id}', '${friend.name.replace(/'/g, "\\'")}', '${friend.avatar}', ${friend.isOnline}); } else { window.goToPage('/profile/${friend.id}/'); }">
