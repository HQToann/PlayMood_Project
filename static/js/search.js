...
let globalSearchData = { songs: [], artists: [], playlists: [], users: [] };
...
async function performSearch(query) {
    try {
        ...
        if (data.success && data.data) {
            ...
            if (songs.length > 0 || artists.length > 0 || playlists.length > 0 || users.length > 0) {
                globalSearchData = { songs, artists, playlists, users };
                applyFilter('all');
...
function renderFilteredResults() {
    const { songs, artists, playlists, users } = globalSearchData;
    ...
    if (currentFilter === 'all') {
        ...
        renderTopResult(songs, uniqueUsers, playlists);
...
function renderTopResult(songs, uniqueUsers, playlists) {
    let topItem = null;
    let topType = '';
        ...
        if (topType === 'artist') {
            ...
            topHtml = `
                <div class="top-result-card p-4 rounded-4 position-relative" style="background-color: var(--bg-card); cursor: pointer; transition: background 0.3s;" onclick="window.location.href='${profileLink}'">
                    ...
                    <button class="btn btn-outline-light rounded-pill fw-bold position-absolute px-4 py-2" style="bottom: 24px; right: 24px;" onclick="event.stopPropagation(); window.toggleFollowUser('${topItem.id}', this);">
                        Theo dõi
                    </button>
                </div>
            `;
    ...
    if (items.length > 0) {
        items.forEach(item => {
            if (type === 'song') {
                ...
                html += `
                    <div class="song-row d-flex align-items-center p-2 rounded-3" style="cursor: pointer;" onclick="window.location.href='/playlist/detail/?id=${item.id}'">
                        <div class="text-white fw-semibold text-truncate" style="font-size: 1rem;">${item.title}</div>
                    </div>
                `;
            }
